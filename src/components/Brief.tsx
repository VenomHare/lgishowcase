  ...
  return (
    <div className={`
      ...
      <ul className="flex flex-col justify-around min-h-[40svh] h-full list-['➦'] w-[80%] lg:w-full">
        <h1 className="text-4xl font-bold text-center mx-auto my-[2svh] text-primary">Features</h1>
        {mod.features.map((feature, index) => (
          <li key={`feature-${index}`} className="text-lg font-semibold text-center mx-auto my-[2svh] lg:my-1">
            {feature}
          </li>
        ))}

        <h1 className="text-4xl font-bold text-center mx-auto my-[2svh] text-primary">Limitations</h1>
        {mod.cons.map((limitation, index) => (
          <li key={`limitation-${index}`} className="text-lg font-semibold text-center mx-auto my-[2svh]">
            {limitation}
          </li>
        ))}
      </ul>
      ...
      <ul className="flex flex-col justify-around min-h-[40svh] h-full list-['➦'] w-[80%] m-5">
        <h1 className="text-5xl font-semibold text-center mx-auto my-[2svh] text-primary">Arena</h1>
        <div className="">
          {mod.arena.map((item, index) => (
            <li key={`arena-${index}`} className="text-lg font-medium text-left ">
              {item}
            </li>
          ))}
        </div>
