

const Brief = () => {
  return (
    <div className='Brief'>
    <h2 className='briefTitle'>Briefly</h2>
      <h5 className='briefSub'>There are three types of patches</h5>
      <div className='briefdetail'>
        1st is the Deluxe Edition patch, this patch is only for  10$ only.
      </div>

      <div className='briefdetail'>
        2nd is a Exclusive edition patch that is chargeable for 18$ only.
      </div>

      <div className='briefdetail'>
        3rd is an Limited Edition patch that is chargeable around 25$ only.
      </div>
      <div className='briefdetail'>
      If you want any exclusive mod in deluxe edition or limited editions patch or separately mod file we charge 6$ for each mod (If you want in the bundle we discount further)
      </div>
  </div>
  )
}

export default Brief