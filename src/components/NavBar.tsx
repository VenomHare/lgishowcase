...
const NavBar: React.FC<Props> = ({ active, isMaintenance = false }) => {
        ...
        <div className='z-[199] fixed top-0 left-0 w-full h-[11%] shadow-2xl flex justify-around items-center bg-nav-background'>
            ...
            `}>
                <img 
                    ...
                    alt='LGI MODz Logo' 
                    className={`max-h-[80%] lg:max-h-full max-w-[70%] ${isMaintenance ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`} 
                    onClick={() => !isMaintenance && navigate("/")} 
                />
            </div>
            ...
            <div className={`
                ...
                {
                    Config.Navbar.map((n, i)=><a key={i} onClick={() => handleNavigation(n.slug)} className='w-full'>
                    <button className={`
